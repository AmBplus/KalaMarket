...
public class ValidationRequestAddSliderServiceDto : AbstractValidator<RequestAddSliderServiceDto>
{
    public ValidationRequestAddSliderServiceDto()
    {
        ...
        RuleFor(x => x.Link).MaximumLength(KalaMarketConstants.MaxLength.Name * 2)
            .WithMessage(ErrorFluentValidation.MaxLen);
        RuleFor(x => x.Src).MaximumLength(KalaMarketConstants.MaxLength.Name * 2)
            .WithMessage(ErrorFluentValidation.MaxLen);
    }
}