...
public static class RemoveFileHelper
{
    ...
    public static async Task RemoveRangeFiles(IWebHostEnvironment hostEnvironment, IList<string> pathFiles, ILoggerManger logger)
    {
        foreach (var pathFile in pathFiles)
        {
            await RemoveFile(hostEnvironment, pathFile,logger);
        }
    }
}